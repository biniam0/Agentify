...
function WizardInner() {
  ...
  useEffect(() => {
    if (state.completed && !isCheckoutReturn) {
      navigate('/app/v2', { replace: true });
    }
  }, [state.completed, navigate, isCheckoutReturn]);

  // Handle cancel return from Stripe — go back to plan selection
  useEffect(() => {
    if (searchParams.get('checkout') === 'cancel') {
      setCurrentStep(3);
      navigate('/app/onboarding', { replace: true });
    }
  }, [searchParams, setCurrentStep, navigate]);
    ...
    <div className="min-h-screen dark:bg-background">
      <div className="min-h-screen flex items-center justify-center px-4 py-10">
        <div className="flex items-start gap-16 w-full max-w-[1060px]">
          ...
          {/* Main content area */}
          <main className={`flex-1 min-w-0 ${isCheckoutReturn ? 'mx-auto max-w-2xl' : ''}`}>
